...
Attributes:
  ...
  expr : Expression. The expression of the function
"""
@dataclass
...
Inherits from Expression
Attributes:
  base : Expression. The base of the exponentiation
  exp : Expression. The exponent
"""
@dataclass
...
class MakeDot(Visitor):
  ...
  def visit(self, f : Function):
    name = self.name()
    self.dot.node(name, label = f'{f.name}()')
    expr = f.expr.accept(self)
    self.dot.edge(name, expr)
    return name
  
  def visit(self, e : Exponentiation):
    name = self.name()
    self.dot.node(name, label = '^')
    base = e.base.accept(self)
    exp = e.exp.accept(self)
    self.dot.edge(name, base)
